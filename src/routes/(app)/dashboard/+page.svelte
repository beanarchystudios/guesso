...
<div class="min-h-full p-6 md:p-10">
	<div class="mx-auto w-full max-w-6xl">
		...
		{:else}
			<div class="grid auto-rows-min gap-4 p-4 sm:grid-cols-2 xl:grid-cols-3">
				{#each courses as course (course.id)}
					<a
						href={resolve(`/courses/${course.id}`)}
						class="block rounded-xl transition-[box-shadow] duration-100 hover:ring-1 hover:ring-(--course)"
						style:--course={colors[`course_${course.id}`] ?? 'transparent'}
					>
						<Card.Root
							class="overflow-hidden p-0 transition-colors hover:border-primary/40 hover:bg-muted/40"
						>
							<Card.Header class="gap-0 p-0">
								...
								{:else}
									...
									></div>
								{/if}
								<Card.Title class="px-6 py-6">{course.name}</Card.Title>
							</Card.Header>
						</Card.Root>
					</a>
				{/each}
